/**
 * components/GraphAfter.tsx
 * =========================
 * React Flow graph showing PSO-optimized edge congestion values.
 */
...
// Shared layout constants (same as GraphBefore.tsx)
...
function buildNodes(): RFNode[] {
  return Object.entries(NODE_POSITIONS).map(([id, position]) => ({
    ...
    type: 'traffic',
    position,
    data: { label: id, isOptimized: true },
  }));
}
...
interface GraphAfterProps {
  /** Optimized edge congestion returned by POST /optimize. Null if not run yet. */
  congestion: Record<string, number> | null;
}

export default function GraphAfter({ congestion }: GraphAfterProps) {
  const nodes = useMemo(() => buildNodes(), []);
  const edges = useMemo(
    () => (congestion ? buildEdges(congestion) : []),
    [congestion],
  );
