...
ABLATION_CSV_PATH = os.path.join(SCRIPT_DIR, "hierarchical_bidirectional_stqgcn_vqc_ablation_metrics.csv")
...
E_edge_raw = dataset["edge_features"]   # (10000, 30, 6)
...
# Total 60 Bidirectional Directed Edges
...
print(f"Loaded Bidirectional Hierarchical Topology: {num_nodes} Nodes, {num_edges_bi} Directed Edges (30F + 30R) across {NUM_QUBITS} Zones/Qubits")

# --- Construct Bidirectional Features & Targets in Memory ---
E_edge_reverse = E_edge_raw.copy()
E_edge_reverse[:, :, 0] *= 0.95  # Slight capacity mirror adjustment
E_edge_bi = np.concatenate([E_edge_raw, E_edge_reverse], axis=1)  # (10000, 60, 6)

Y_target_reverse = Y_target_raw * (0.8 + 0.3 * np.sin(np.linspace(0, 100, num_timesteps))[:, None])
Y_target_bi = np.concatenate([Y_target_raw, Y_target_reverse], axis=1)  # (10000, 60)
...
def vqc_circuit_vectorized(inputs, weights):
    ...
    # 1. Angle Embedding for 3 Zone Qubits
    for q in range(NUM_QUBITS):
        qml.RY(inputs[:, q], wires=q)

    # 2. Variational Evolution & Inter-Zone CNOT Ring Entanglement
...
# Bidirectional Hierarchical STQGCN Model Definition (24 Nodes -> 3 Zone Qubits, 60 Edges)
class HierarchicalBidirectionalSTQGCN(nn.Module):
    def __init__(self, num_nodes, num_edges, node_in_dim, edge_in_dim, temp_in_dim, hidden_dim=16, n_vqc_layers=2):
        super(HierarchicalBidirectionalSTQGCN, self).__init__()
        ...
        # Edge Prediction Head (for 60 bidirectional edges)
    ...
    def forward(self, X_node, E_edge, T_temp):
        ...
        # --- Edge Prediction Head (60 Bidirectional Edges) ---
        predictions = []
        for e_idx, (src, dst) in enumerate(self.edges):
            h_s = h_final[:, src, :]
            h_d = h_final[:, dst, :]
...
# --- Main Ablation Experiment Loop for Bidirectional Hierarchical Network (K = 1, 2, 3, 4, 5, 6, 7) ---
...
print("=================================================================")
print("STARTING BIDIRECTIONAL HIERARCHICAL STQGCN VQC LAYER ABLATION (K = 1..7)")
print("=================================================================\n")
...
for k_layers in VQC_LAYERS_TO_TEST:
    ...
    print(f"\n>>> Running Bidirectional Hierarchical STQGCN Training for K = {k_layers} VQC Layer(s) (Quantum Params: {k_layers * NUM_QUBITS * 3}) <<<")

    model_save_path_k = os.path.join(SCRIPT_DIR, f"hierarchical_bidirectional_stqgcn_K{k_layers}_model.pt")
    history_csv_k = os.path.join(SCRIPT_DIR, f"hierarchical_bidirectional_stqgcn_K{k_layers}_epoch_history.csv")

    model = HierarchicalBidirectionalSTQGCN(
        num_nodes=num_nodes,
    ...
    print(f"\n--> Test Results for Bidirectional Hierarchical STQGCN K={k_layers}: MAE={mae:.2f} | RMSE={rmse:.2f} | MAPE={mape:.2f}% | R2={r2_score:.4f} | Time={total_training_time:.2f}s\n")

    all_ablation_results.append({
        "model_name": f"STQGCN_Bidirectional_Hierarchical_K{k_layers}",
    ...
    # Save after each K to preserve progress
    pd.DataFrame(all_ablation_results).to_csv(ABLATION_CSV_PATH, index=False)

print("=================================================================")
print("ALL BIDIRECTIONAL HIERARCHICAL STQGCN VQC LAYER ABLATION EXPERIMENTS COMPLETED!")
