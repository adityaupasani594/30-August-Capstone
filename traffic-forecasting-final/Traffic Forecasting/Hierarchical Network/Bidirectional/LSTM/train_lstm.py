...
METRICS_CSV_PATH = os.path.join(SCRIPT_DIR, "hierarchical_bidirectional_lstm_performance_metrics.csv")
HISTORY_CSV_PATH = os.path.join(SCRIPT_DIR, "hierarchical_bidirectional_lstm_epoch_history.csv")
MODEL_SAVE_PATH = os.path.join(SCRIPT_DIR, "hierarchical_bidirectional_lstm_model.pt")
...
print(f"Bidirectional Hierarchical Topology: {num_nodes} Nodes, {num_edges_bi} Directed Edges (30F + 30R)")
...
# LSTM input: [node_features_flat | edge_features_flat | temporal_features]
# Shape: (10000, num_nodes*node_feat_dim + num_edges*edge_feat_dim + temp_feat_dim)
X_node_flat = X_node_norm.reshape(num_timesteps, -1)       # (10000, 24*6=144)
E_edge_flat = E_edge_norm.reshape(num_timesteps, -1)       # (10000, 60*6=360)
input_features = np.concatenate([X_node_flat, E_edge_flat, T_temp], axis=1)  # (10000, 508)
...
# --- LSTM Model ---
# Architecture: 2-layer LSTM -> Linear output head
# Input reshaped to (batch, seq_len=1, input_dim) for single-step prediction
class TrafficLSTM(nn.Module):
    ...
    def forward(self, x):
        # x: (batch, input_dim) -> reshape to (batch, seq_len=1, input_dim)
        x = x.unsqueeze(1)
        lstm_out, _ = self.lstm(x)           # (batch, 1, hidden_dim)
        out = lstm_out[:, -1, :]             # (batch, hidden_dim)
        return self.head(out)                # (batch, output_dim=60)
...
# --- Training Loop ---
NUM_EPOCHS = 40
print(f"\nStarting Bidirectional Hierarchical LSTM Training for {NUM_EPOCHS} Epochs...")
...
print("\n======== BIDIRECTIONAL HIERARCHICAL LSTM TEST METRICS ========")
print(f"Model Architecture    : 2-Layer LSTM (Bidirectional Hierarchical Network)")
...
metrics_summary = [{
    "model_name": "Forecast_5s_LSTM_Bidirectional_Hierarchical_24N",
