...
METRICS_CSV_PATH = os.path.join(SCRIPT_DIR, "hierarchical_unidirectional_lstm_performance_metrics.csv")
HISTORY_CSV_PATH = os.path.join(SCRIPT_DIR, "hierarchical_unidirectional_lstm_epoch_history.csv")
MODEL_SAVE_PATH = os.path.join(SCRIPT_DIR, "hierarchical_unidirectional_lstm_model.pt")
...
print(f"Unidirectional Hierarchical Topology: {num_nodes} Nodes, {num_edges} Directed Edges")

# Normalization
mu_traffic = float(np.mean(Y_target_raw))
std_traffic = float(np.std(Y_target_raw))
print(f"Target Flow Normalization Stats: mean = {mu_traffic:.2f}, std = {std_traffic:.2f}")
...
# Shape: (10000, 24*6 + 30*6 + 4) = (10000, 328)
X_node_flat = X_node_norm.reshape(num_timesteps, -1)       # (10000, 144)
E_edge_flat = E_edge_norm.reshape(num_timesteps, -1)       # (10000, 180)
input_features = np.concatenate([X_node_flat, E_edge_flat, T_temp], axis=1)  # (10000, 328)
input_dim = input_features.shape[1]

print(f"LSTM Input Dimension: {input_dim} (node:{num_nodes*node_feat_dim} + edge:{num_edges*edge_feat_dim} + temp:{temp_feat_dim})")
...
class TrafficLSTM(nn.Module):
    ...
    def forward(self, x):
        # x: (batch, input_dim) -> (batch, seq_len=1, input_dim)
        x = x.unsqueeze(1)
...
print(f"\nLSTM Model: hidden_dim={HIDDEN_DIM}, num_layers={NUM_LAYERS}, output_dim={num_edges}")
...
# --- Training Loop ---
NUM_EPOCHS = 40
print(f"\nStarting Unidirectional Hierarchical LSTM Training for {NUM_EPOCHS} Epochs...")
...
print("\n======== UNIDIRECTIONAL HIERARCHICAL LSTM TEST METRICS ========")
print(f"Model Architecture    : 2-Layer LSTM (Unidirectional Hierarchical Network)")
print(f"Network Topology      : 24 Nodes (Tree + Mesh + Linear), 30 Directed Edges")
...
metrics_summary = [{
    "model_name": "Forecast_5s_LSTM_Unidirectional_Hierarchical_24N",
    "network_type": "Hierarchical_Network_Unidirectional",
    "scale": "24_Nodes_3_Zones_30_Edges",
    "model_type": "LSTM",
    "hidden_dim": HIDDEN_DIM,
    "num_layers": NUM_LAYERS,
