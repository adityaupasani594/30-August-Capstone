...
# Output CSV paths
METRICS_CSV_PATH = os.path.join(SCRIPT_DIR, "hierarchical_classical_stgcn_performance_metrics.csv")
HISTORY_CSV_PATH = os.path.join(SCRIPT_DIR, "hierarchical_classical_stgcn_epoch_history.csv")
MODEL_SAVE_PATH = os.path.join(SCRIPT_DIR, "hierarchical_classical_stgcn_model.pt")
...
print(f"Loaded Hierarchical Topology: {num_nodes} Nodes, {num_edges} Directed Edges across {NUM_ZONES} Zones")
print(f"  - Zone 1 (Tree): Nodes {ZONE_1_NODES}")
print(f"  - Zone 2 (Mesh): Nodes {ZONE_2_NODES}")
print(f"  - Zone 3 (Linear): Nodes {ZONE_3_NODES}")
...
class HierarchicalClassicalSTGCN(nn.Module):
    ...
    def forward(self, X_node, E_edge, T_temp):
        ...
        # Pool node embeddings into 3 Zone Super-Nodes
        ...
        zone_out = self.zone_update(torch.cat([zones_embed, T_zone_expanded], dim=-1)) # (B, 3, hidden_dim)
        
        # Broadcast updated zone context back to Tier 1 nodes
...
# --- Training Loop ---
NUM_EPOCHS = 40
print(f"\nStarting Hierarchical Classical STGCN Training for 24-Node Network ({num_edges} Edges) for {NUM_EPOCHS} Epochs...")
...
print("\n================ UNIDIRECTIONAL HIERARCHICAL CLASSICAL STGCN TEST METRICS ================")
print(f"Model Architecture : Classical STGCN (Unidirectional Hierarchical Network)")
...
metrics_summary = [{
    "model_name": "Forecast_5s_Classical_STGCN_Unidirectional_Hierarchical_24N",
