...
ABLATION_CSV_PATH = os.path.join(SCRIPT_DIR, "stqgcn_bidirectional_vqc_ablation_metrics.csv")
...
# Ultra-Fast PyTorch Quantum VQC Module
...
# Dynamic STQGCN Model Definition for Bidirectional Graph
class STQGCN(nn.Module):
    def __init__(self, num_nodes, num_edges, node_in_dim, edge_in_dim, temp_in_dim, hidden_dim=16, n_qubits=8, n_vqc_layers=2):
        ...
        # 5. Edge Prediction Head (Eq 15) for 22 Edges
    ...
    def forward(self, X_node, E_edge, T_temp):
        ...
        # CPU Device Bridge for PennyLane QNode Execution
...
# --- Main Ablation Experiment Loop for Bidirectional Network (K = 1, 2, 3, 4, 5) ---
...
print("=================================================================")
print("STARTING BIDIRECTIONAL STQGCN VQC LAYER ABLATION (K = 1, 2, 3, 4, 5, 6)")
print("=================================================================\n")
...
for k_layers in VQC_LAYERS_TO_TEST:
    ...
    print(f"\n>>> Running Bidirectional STQGCN Training for K = {k_layers} VQC Layer(s) (Params: {k_layers * NUM_QUBITS * 3}) <<<")
    
    model_save_path_k = os.path.join(SCRIPT_DIR, f"stqgcn_bidirectional_K{k_layers}_model.pt")
    history_csv_k = os.path.join(SCRIPT_DIR, f"stqgcn_bidirectional_K{k_layers}_epoch_history.csv")
    ...
    print(f"\n--> Test Results for Bidirectional STQGCN K={k_layers}: MAE={mae:.2f} | RMSE={rmse:.2f} | MAPE={mape:.2f}% | R2={r2_score:.4f} | Time={total_training_time:.2f}s\n")

    all_ablation_results.append({
        "model_name": f"STQGCN_Bidirectional_K{k_layers}",
...
print("=================================================================")
print("ALL 5 BIDIRECTIONAL VQC LAYER ABLATION EXPERIMENTS COMPLETED!")
