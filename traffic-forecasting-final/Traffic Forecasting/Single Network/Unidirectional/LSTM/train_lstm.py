...
METRICS_CSV_PATH = os.path.join(SCRIPT_DIR, "single_unidirectional_lstm_performance_metrics.csv")
HISTORY_CSV_PATH = os.path.join(SCRIPT_DIR, "single_unidirectional_lstm_epoch_history.csv")
MODEL_SAVE_PATH = os.path.join(SCRIPT_DIR, "single_unidirectional_lstm_model.pt")
...
print(f"Unidirectional Single Network Topology: {num_nodes} Nodes, {num_edges} Directed Edges")
...
# Shape: (10000, 8*6 + 11*6 + 4) = (10000, 118)
X_node_flat = X_node_norm.reshape(num_timesteps, -1)       # (10000, 48)
E_edge_flat = E_edge_norm.reshape(num_timesteps, -1)       # (10000, 66)
input_features = np.concatenate([X_node_flat, E_edge_flat, T_temp], axis=1)  # (10000, 118)
...
# --- Training Loop ---
NUM_EPOCHS = 40
print(f"\nStarting Unidirectional Single Network LSTM Training for {NUM_EPOCHS} Epochs...")
...
print("\n======== UNIDIRECTIONAL SINGLE NETWORK LSTM TEST METRICS ========")
print(f"Model Architecture    : 2-Layer LSTM (Unidirectional Single Network)")
print(f"Network Topology      : 8 Nodes, 11 Directed Edges")
...
metrics_summary = [{
    "model_name": "Forecast_5s_LSTM_Unidirectional_Single_8N",
    "network_type": "Single_Network_Unidirectional",
    "scale": "8_Nodes_11_Edges",
    "model_type": "LSTM",
    "hidden_dim": HIDDEN_DIM,
    "num_layers": NUM_LAYERS,
